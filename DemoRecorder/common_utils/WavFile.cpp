...
static inline float _swap32f(float &dwData)
{
    // do nothing
    return dwData;
}
...
int WavInFile::_readPcmFile(FILE *file ,int sampleRate,int bits,int channels){
    fseek(file, 0, SEEK_END);
    int filelen =  (int)ftell(file);
    fseek(file, 0, SEEK_SET);

    {//set fake header info
        memset(&header, 0, sizeof(header));

        //构造riff
        memcpy(header.riff.riff_char,riffStr,4);
        header.riff.package_len = filelen + sizeof(WavHeader) - sizeof(WavRiff) + 4;;//文件的总字节数,不包括标志符4字节以及文件长度本身所占用的4字节。
        ...
        if(bits==32){
            header.format.fixed=3;//32位时，默认表示使用float类型未压缩格式来存储
        }
        header.format.channel_number=channels;
        header.format.sample_rate=sampleRate;
        header.format.byte_rate = sampleRate*bits/8*channels;
        header.format.byte_per_sample = bits/8*channels;
        header.format.bits_per_sample = bits;

        //构造data
        memcpy(header.data.data_field, dataStr, 4);
        header.data.data_len = filelen;
    }
                        ...
                        int channels)
{
    ...
    if (sampleRate==0 && bits==0 && channels==0){
        ...
        _readPcmFile(fptr,sampleRate,bits,channels);
        isReadPcmFile= true;
    }
...
WavInFile::WavInFile(const char *filename,
    ...
    std::string  tstr(filename);
    if (tstr.find(".wav")!=string::npos){//给的是wav文件名，按照wav读取方式读取
        // Try to open the file for reading
        init();
    }else{
        assert(bits%2==0 && bits<=32);
        isReadPcmFile = true;
        _readPcmFile(fptr,sampleRate,bits,channels);
    }
}
...
int WavInFile::seek(long samplesPosition){
    long seekBytes = samplesPosition * header.format.bits_per_sample/8 ;
    assert( seekBytes>=0 && seekBytes<getDataSizeInBytes());
    int ret =  fseek(fptr, seekBytes-dataRead, SEEK_CUR);
    if(ret==0){
        dataRead = seekBytes;
    }
    return ret;
...
int WavInFile::read(short *buffer, int maxElems)
{
    ...
    case 32:
    {
        ...
        int numBytes = maxElems * 4;
        if(numBytes<=0){
            return 0;
        }
        void* buf1 = getConvBuffer(numBytes);
        numBytes = (int)fread(buf1, 1, numBytes, fptr);
        dataRead += numBytes;
        numElems = numBytes / bytesPerSample;

        if(header.format.fixed==3){

            float *temp = (float*)buf1;

            for (int i = 0; i < numElems; i ++)
            {
                buffer[i] = (short)( temp[i] * 32767.0f);
            }
        }else{
            int* temp = (int*)buf1;
            for (int i = 0; i < numElems; i ++)
            {
                buffer[i] = (short)( temp[i] );
            }
        }
...
int WavInFile::read(short *buffer, long startSampleIndex, long endSampleIndex) {
    
    assert(startSampleIndex<=endSampleIndex);
 
    int numElems = 0;
    int maxElems = (int)(endSampleIndex - startSampleIndex + 1);
    assert(buffer);
    switch (header.format.bits_per_sample)
    {
    case 8:
    {
        int startIndexInByte = (int)startSampleIndex;
        int endIndexInByte = (int)endSampleIndex;

        int numBytes = (endIndexInByte - startIndexInByte + 1);
        if (numBytes > maxElems) numBytes = maxElems;
        if(numBytes<=0){
            numElems=0;
            break;
        }
        assert( startIndexInByte>0 && endIndexInByte<getDataSizeInBytes());
        //移到数据开始的地方
        fseek(fptr, dataStartPosition, SEEK_SET);
        //移到对应读取的位置
        fseek(fptr, startIndexInByte, SEEK_CUR);
        
        // 8 bit format
        unsigned char *temp = (unsigned char*)getConvBuffer(maxElems);
        int i;
        numBytes = (int)fread(temp, 1, numBytes, fptr);
        dataRead += startIndexInByte+numBytes;
        numElems = numBytes;
        // convert from 8 to 16 bit
        for (i = 0; i < numElems; i ++)
        {
    ...
    case 16:
    {
        ...
        int startIndexInShort = (int)startSampleIndex;
        int endIndexInShort = (int)endSampleIndex;

        int numBytes = (endIndexInShort - startIndexInShort + 1) * 2;
        if (numBytes > maxElems * 2) numBytes = maxElems * 2;
        if(numBytes<=0){
            numElems=0;
            break;
        }
        assert( (startIndexInShort*2>=0) && (endIndexInShort*2<getDataSizeInBytes()) );
        //移到数据开始的地方
        fseek(fptr, dataStartPosition, SEEK_SET);
        //移到对应读取的位置
        fseek(fptr, startIndexInShort * 2, SEEK_CUR);
        
        //numBytes = maxElems * 2;
        numBytes = (int)fread(buffer, 1, numBytes, fptr);
        dataRead += startIndexInShort*2+numBytes;
        numElems = numBytes / 2;
    ...
    case 32:
    {
        ...
        int startIndexInByte = (int)startSampleIndex*bytesPerSample;
        int endIndexInByte = (int)endSampleIndex*bytesPerSample;

        int numBytes = (endIndexInByte - startIndexInByte + 1);
        if (numBytes > maxElems*bytesPerSample) numBytes = maxElems*bytesPerSample;
        if(numBytes<=0){
            numElems=0;
            break;
        }
...
int WavInFile::read(short *buffer, long startTime, long endTime, int maxElems) {
    //long afterDataRead;
    //int numBytes;
    int numElems=0;
    ...
    case 8:
    {
        int startIndexInByte = round(startTime * 0.001 * getSampleRate() * getNumChannels());
        int endIndexInByte = round(endTime * 0.001 * getSampleRate() * getNumChannels() - 1);
    ...
    case 16:
    {
        ...
        int startIndexInShort = round(startTime * 0.001 * getSampleRate() * getNumChannels());
        int endIndexInShort = round(endTime * 0.001 * getSampleRate() * getNumChannels() - 1);
    ...
    case 32:
    {
        ...
        int startIndexInByte = (int)round(startTime * 0.001 * getSampleRate() * getNumChannels())*bytesPerSample;
        int endIndexInByte = (int)round(endTime * 0.001 * getSampleRate() * getNumChannels() - 1)*bytesPerSample;
...
int WavInFile::read(float *buffer, long startTime, long endTime, int maxElems) {
    ...
    assert(buffer);
    int startIndexBySample = round(startTime * 0.001 * getSampleRate() * getNumChannels());
    int endIndexBySample = round(endTime * 0.001 * getSampleRate() * getNumChannels() - 1);

    int numBytes = (startIndexBySample - endIndexBySample + 1) * bytesPerSample;
    if(numBytes<=0){
        return 0;
    }
    if (numBytes > maxElems * bytesPerSample) numBytes = maxElems * bytesPerSample;
    fseek(fptr, startIndexBySample * bytesPerSample, SEEK_SET);

    char *temp = (char*)getConvBuffer(numBytes);

    numBytes = (int)fread(temp, 1, numBytes, fptr);
    dataRead += startIndexBySample*bytesPerSample+numBytes;
    numElems = numBytes / bytesPerSample;
...
int WavInFile::readWavHeaders()
{
    ...
    dataStartPosition = (int)ftell(fptr);
...
WavOutFile::WavOutFile(FILE *file, int sampleRate, int bits, int channels,bool pcmonly)
{
    assert(bits%2==0 && bits<=32);
    bytesWritten = 0;
    fptr = file;
...
void WavOutFile::finishHeader()
{
    ...
    header.data.data_len = (uint)bytesWritten;
    header.fact.fact_sample_len = (uint)(bytesWritten / header.format.byte_per_sample);
    if(!writePcmOnly){
        writeHeader();
    }
}
...
void WavOutFile::write(const short *buffer, int numElems)
{
    ...
    {
        // use temp buffer to swap byte order if necessary
        float *pTemp = (float *)getConvBuffer(numElems * sizeof(float));
        float conv = 1.0/32768.0;
        for(int i=0;i<numElems;i++){
            pTemp[i] = buffer[i] * conv;
        }

        //_swap32fBuffer(pTemp, numElems); TODO 32f支持大端模式

        res = (int)fwrite(pTemp, 4, numElems, fptr);

        if (res != numElems)
        {
            ST_THROW_RT_ERROR("Error while writing to a wav file.");
        }
        bytesWritten += 4 * numElems;
        break;
    }
    default:
    {
