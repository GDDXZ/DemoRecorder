...
static const int kMaximumCallbacksPerSource            = 50;
...
static OSStatus renderCallback(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
    ...
    if (THIS.pause || channel == NULL || channel->ptr == NULL || !channel->playing ) {
...
//  NSAssert(__AEAllocated == NO, @"You may only use one TAAE instance at a time");
...
{
    if (self.housekeepingTimer) {
        [self.housekeepingTimer invalidate];
        self.housekeepingTimer = nil;
    }
    
    self.housekeepingTimer = [NSTimer timerWithTimeInterval:time
                                                     target:[[AEAudioControllerProxy alloc] initWithAudioController:self]
...
-(void)setAudioSessionCategory:(NSString *)audioSessionCategory {
    ...
    if (@available(iOS 10.0, *)) {
        options |= (_inputEnabled && _enableBluetoothInput) ? AVAudioSessionCategoryOptionAllowBluetoothA2DP : 0;
    } else {
        options |= (_inputEnabled && _enableBluetoothInput) ? AVAudioSessionCategoryOptionAllowBluetooth : 0;
    }
#endif
    ...
    if ([_audioSessionCategory isEqualToString:AVAudioSessionCategoryPlayAndRecord]) {
        for (AVAudioSessionPortDescription *destPort in audioSession.availableInputs){
            if ([destPort.portType isEqualToString:AVAudioSessionPortUSBAudio]) {
                [audioSession setPreferredInput:destPort error:nil];
                break;
            }
        }
    }
...
-(void)setUseMeasurementModeForce:(BOOL)useMeasurementModeForce
{
    _useMeasurementModeForce = useMeasurementModeForce;
    
    if (useMeasurementModeForce) {
        self.useMeasurementMode = YES;
    }
}
...
-(void)setUseMeasurementMode:(BOOL)useMeasurementMode {
    ...
    NSError *error = nil;
    NSString *mode = _useMeasurementMode && (!_avoidMeasurementModeForBuiltInSpeaker || !_playingThroughDeviceSpeaker) ? AVAudioSessionModeMeasurement : AVAudioSessionModeDefault;
    if (_useMeasurementModeForce) {
    ...
    if ( context == kChannelPropertyChanged ) {
        ...
        if ( [keyPath isEqualToString:@"volume"] ) {
            ...
            if ( group->mixerAudioUnit ) {
                ...
                AECheckOSStatus(result, "AudioUnitSetParameter(kMultiChannelMixerParam_Enable)");
                if ([channel isKindOfClass:[AEAudioFilePlayer class]]) {
                    AEAudioFilePlayer *player = channel;
                    if (player.playingChangedBlock) {
                        player.playingChangedBlock(channel.channelIsPlaying);
                    }
                }
            }
            ...
            if ( group->mixerAudioUnit ) {
                OSStatus result;
                AudioStreamBasicDescription zero = {0};
                if (!memcmp(&channelElement->audioDescription, &zero, sizeof(AudioStreamBasicDescription))) {
                    result = AudioUnitSetProperty(group->mixerAudioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, index,
                                                  &_audioDescription, sizeof(AudioStreamBasicDescription));
                } else {
                    result = AudioUnitSetProperty(group->mixerAudioUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, index,
                                                  &channelElement->audioDescription, sizeof(AudioStreamBasicDescription));
                }
...
- (void)interruptionNotification:(NSNotification*)notification {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (interruptionType == AVAudioSessionInterruptionTypeEnded ) {
            ...
            if ( applicationState != UIApplicationStateBackground || _started ) {
            ...
            if (iaaConnected && _started && !self.running && [self start:NULL]) {
                return;
            }
            if (applicationState == UIApplicationStateActive && _started && !self.running ) {
                self.allowMixingWithOtherApps = YES;
                if ([self start:NULL]) {
                    return;
                }
            }
...
- (void)audioRouteChangeNotification:(NSNotification*)notification {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ( (_useMeasurementMode && _avoidMeasurementModeForBuiltInSpeaker) || _useMeasurementModeForce) {
            NSError *error = nil;
            NSString *mode = !_playingThroughDeviceSpeaker ? AVAudioSessionModeMeasurement : AVAudioSessionModeDefault;
            if (_useMeasurementModeForce) {
                mode = AVAudioSessionModeMeasurement;
            }
            if ( ![audioSession setMode:mode error:&error] ) {
